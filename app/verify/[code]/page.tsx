...
export async function generateMetadata({
  ...
  const { data: cert } = await admin
    ...
    .eq("code", params.code)
    .maybeSingle();
  if (!cert) return { title: "Certificate · SparkLine" };
  const user = Array.isArray(cert.user) ? cert.user[0] : cert.user;
  return {
    title: `${user?.full_name ?? "Graduate"} · SparkLine certificate`,
    openGraph: {
    ...
    <div className="min-h-screen bg-black text-white">
      <main className="mx-auto max-w-2xl px-5 py-16 md:px-8">
        ...
        <div className="mt-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/55">
              This certificate is genuine and issued by SparkLine.
            </p>
          </div>
        </div>
        <Card className="mt-8">
        ...
        <p className="mt-6 text-center text-xs text-white/40">
          Verify any SparkLine certificate at sparkline.app/verify/&lt;code&gt;
        </p>
      </main>
    </div>
  );
