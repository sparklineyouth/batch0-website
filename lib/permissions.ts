...
export const ADMIN_ROUTE_PERMISSIONS: ReadonlyArray<readonly [string, Permission]> = [
  ...
  ["/admin/students/new", "people.manage"],
...
const LIVE_ROOM_PATH = /^\/dashboard\/(events|calls)\/[^/]+\/live\/?$/;

/**
 * Is this path a live room — a webinar or a 1:1 — rather than part of the
 * student dashboard it happens to be filed under?
 *
 * The rooms live at /dashboard/events/<id>/live and /dashboard/calls/<id>/live,
 * but most of the people who HOST them cannot open /dashboard: mentors and
 * investors (who book 1:1s), interns and custom staff roles with Manage events,
 * and mentor or investor guest speakers all lack `student.dashboard`. The
 * middleware exempts exactly these two paths from the student.dashboard
 * redirect and from the pre-cohort lockdown, and that is safe only because
 * both pages authorize themselves: the webinar room through the caller's RLS
 * read (lib/live-access.ts), the 1:1 through the "are you one of the two"
 * party check. Anything deeper (`…/live/anything`) or shallower (the event
 * page itself) is NOT a room and keeps the normal gate.
 */
export function isLiveRoomPath(path: string): boolean {
  return LIVE_ROOM_PATH.test(path);
}

/**
 * Where a 1:1's OWNER (the inviter) manages their calls — the Back link out of
 * a call they booked, and the link in "they accepted your 1:1".
 *
 * By panel, most specific first: the admin calls page if they may open it,
 * then the mentor panel, then the investor panel. Falls back to the student
 * view, which is where an invitee's Back goes anyway. Sending every host to
 * /dashboard/calls used to bounce mentors to /mentor and show admins a page
 * listing only calls where THEY were the invitee, so their call seemed to have
 * vanished.
 */
export function callsHomeFor(caps: Capabilities | null): string {
  if (canViewAdminPath(caps, "/admin/calls")) return "/admin/calls";
  if (can(caps, "mentor.panel")) return "/mentor/calls";
  if (can(caps, "investor.panel")) return "/investor/calls";
