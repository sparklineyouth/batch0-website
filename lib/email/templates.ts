 ...
 * the raw value only for an unparseable date (never a real row). Unescaped —
 * the layout escapes a preheader itself; a body escapes it at the call site.
 */
function eventWhen(startsAt: string): string {
  return formatEasternDateTime(startsAt) ?? startsAt;
}
...
One account, one pass — the code stops working the moment it's claimed.`,
  ...
  /**
   * An event is coming up. The time is rendered in Eastern with its zone
   * named (formatEasternDateTime, the same as the 1:1 invite): this runs on
   * the server, where `toLocaleString()` meant UTC with no label — hours out
   * for every reader, with nothing to say so.
   */
  eventReminder: (args: {
    title: string;
    startsAt: string;
    ...
    html: layout({
      preheader: eventWhen(args.startsAt),
      body: `
        <h1 style="margin:0 0 12px 0;font-size:20px;color:#fff">${escape(args.title)}</h1>
        <p>Starts <strong>${escape(eventWhen(args.startsAt))}</strong>.</p>
   ...
   *
   * The start time names its zone (formatEasternDateTime). A guest is the one
  ...
  cohortDigest: (args: {
    ...
    const eventItems = args.upcomingEvents
      .map(
        (e) =>
          `<li style="margin:0 0 6px 0;color:#bbb">${escape(e.title)} · <span style="color:#888">${escape(eventWhen(e.startsAt))}</span></li>`,
      )
      .join("");
    return {
